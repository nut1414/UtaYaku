    ...
    >
      <div class="w-full flex gap-10 text-center justify-center">
        ...
        <div class="flex flex-col gap-1">
          ...
          <button
            class="h-full rounded-lg hover:cursor-pointer hover:bg-gray-900 border-2"
            @click="getMetaData"
          >
            Search
          </button>
      ...
      <div class="w-full flex flex-col overflow-scroll gap-5">
        ...
        >
          ...
          <h1 class="text-[#C5D3E8]">Artist: {{ candidate.artistName }}</h1>
          <h1 class="text-[#FFDDAE]">Album: {{ candidate.albumName }}</h1>
          <a
            :href="
              candidate.id +
              '?song_name=' +
          ...
          >
            <h1
              class="text-white hover:outline bg-transparent px-2 hover:outline-slate-300 w-full text-right rounded-lg"
            >
              Play
            </h1>
          </a>
...
const getMetaData = async () => {
  ...
  const lyricsResponse = await fetch(
    `https://lrclib.net/api/search?track_name=${song_name.value}&artist_name=${artist_name.value}`
  );
